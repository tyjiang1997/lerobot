...
class JoyCon:
    ...
    def __init__(self, vendor_id: int, product_id: int, serial: str = None, simple_mode=False):
        ...
        if serial is None:
            raise ValueError('serial is required (got None). Did device enumeration fail?')
        ...
        # connect to joycon
        self.enable = True
        # Use the enumerated serial to disambiguate devices.
    ...
    def _open(self, vendor_id, product_id, serial):
        try:
            ...
            candidates = []
            try:
                for d in hid.enumerate(JOYCON_VENDOR_ID, 0):
                    if d.get("product_id") in JOYCON_PRODUCT_IDS:
                        candidates.append(
                            {
            ...
            raise OSError(
                "joycon connect failed (hid open failed). "
                f"Requested vendor_id={vendor_id!r} product_id={product_id!r} serial={serial!r}. "
                f"Enumerated joycon candidates={candidates!r}"
            ) from e
